...
function LazyXerath:useWOnMinion()
	if Game.CanUseSpell(_W) == 0 and castSpell.state == 0 then
		for i = 1, Game.MinionCount() do
		local minion = Game.Minion(i)
		
			if self.lastTarget == nil then self.lastTarget = minion end
			if minion and (minion == self.lastTarget or (GetDistance(minion.pos,self.lastTarget.pos) > 400 and GetTickCount() - self.lastTarget_tick > LazyMenu.TargetSwitchDelay:Value())) then
				local WSpellData = {speed = huge,	range = 1000, 	delay = 0.5, 	radius = 200, 	collision = {nil}, 		type = "circular"}
				local wPred = _G.PremiumPrediction:GetPrediction(myHero, minion, WSpellData)
				if wPred.CastPos then
					self:useWdash(minion)
					self:useWCC(minion)
					self:useWkill(minion,wPred.CastPos)
					self:useWhighHit(minion,wPred.CastPos)
				end
			end
